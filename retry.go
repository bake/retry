...
// VerifierFunc can be passed to `WithVerifier` to specify if a request should
// be tried again based on its response. If it returns true, the request might
// be repeated.
...
func WithVerifier(fn VerifierFunc) func(*roundTripper) {
	return func(rt *roundTripper) { rt.verifier = fn }
}
...
// It fails when the underlying `http.RoundTripper` returns an error.
func (t *roundTripper) RoundTrip(req *http.Request) (res *http.Response, err error) {
	for i := 0; i < t.retries; i++ {
		fmt.Println("try", i, req.URL.String())
